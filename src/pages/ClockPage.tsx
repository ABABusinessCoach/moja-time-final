...
import { LogIn, LogOut, Delete, RotateCcw, User } from 'lucide-react';
...
export function ClockPage() {
  ...
  async function lookupStaff(pinValue: string) {
    ...
    setLookingUp(false);
    if (result.success) {
      setStaffInfo({ staff_name: result.staff_name, is_clocked_in: result.is_clocked_in });
    } else {
  ...
  async function handleClock(action: 'in' | 'out') {
    ...
    if (result.success) {
      ...
      setToast({ message: `${result.staff_name} - ${actionLabel} at ${time}`, type: 'success' });
      setPin('');
      setStaffInfo(null);
      lookupDone.current = false;
    } else {
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            {staffInfo && (
              <div className={`px-5 py-4 border-b flex items-center gap-3 animate-fade-in ${
                staffInfo.is_clocked_in ? 'bg-green-50 border-green-100' : 'bg-moja-bg border-gray-100'
              }`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  staffInfo.is_clocked_in ? 'bg-green-100' : 'bg-moja-blue/10'
                }`}>
                ...
                <div>
                  ...
                  <p className={`text-xs font-semibold ${
                    staffInfo.is_clocked_in ? 'text-green-600' : 'text-moja-blue/50'
                  }`}>
                    {staffInfo.is_clocked_in ? 'Currently clocked in' : 'Currently clocked out'}
                  </p>
                </div>
              </div>
            )}
